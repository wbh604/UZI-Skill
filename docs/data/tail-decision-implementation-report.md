...
- 账户层已改为 ETF 与隔夜个股统一择优，最终最多生成一个模拟操作；没有合格候选时空仓。
- 本地历史窗口、无凭据双源行情、前向快照、公告风险、ETF 异常价格代理和股票交易状态已接入正式生产网关。
- `final`、`close`、`exit_open`、`exit_check` 已接入只追加的模拟持仓账本，股票退出执行 T+1，不代表真实成交。
- 前向验证日志与 ETF／个股／组合分项统计已实现，60 个交易日、40 笔模拟入场、收益、Profit Factor、回撤和行情证据门槛不可自动放宽。
- 正式前向起始日：`not_started:尚未完成无 Token 生产 final 冒烟及本地调度启用`。只有首个生产就绪的交易日 `final` 记录和可用账本同时落盘后才开始计数。

## 2026-08-04 调度与生产冒烟

- 无 Token 真实 `warmup` 已运行：免费源只返回上一交易日行情，系统正确输出 `blocked`、敞口 0、账本事件 0，并保存行情快照和阻断报告。
- 盘前来源时间戳跨日问题已修复为按上海日期分组追加；陈旧行情仍由质量门阻断，不会被转换成当天尾盘特征。
- 七个 Windows 本地任务已实际注册：09:25、09:35、14:00、14:10、14:20、14:30、15:05；健康检查全部通过。
- 正式前向起始日仍为 `not_started:尚无生产就绪的交易日 final 记录`。盘前安全阻断不计入 60 个交易日起点。
